...
def get_vectorstore(text_chunks: Optional[List[str]]):
    ...
    """
    try:
        logger.info(f"Initializing BedrockEmbeddings in region: {aws_region}")
        ...
        if text_chunks is None:
            logger.info(f"Initialized Vector store in region: {aws_region}")
            return PGVector(
...
if __name__ == '__main__':
    try:
        ...
        connection = f"postgresql+psycopg://{os.getenv('PGVECTOR_USER')}:{os.getenv('PGVECTOR_PASSWORD')}@{os.getenv('PGVECTOR_HOST')}:{os.getenv('PGVECTOR_PORT')}/{os.getenv('PGVECTOR_DATABASE')}"
